...
# run model
objModel <- train(trainDF[,predictorsNames], trainDF[,outcomeName], method='gbm',  metric = "RMSE", trControl=objControl)
...
#Create resampling object for train function
objControl <- trainControl(method='cv', number=3, returnResamp='none')

#Train model
objModel <- train(df_train[,predictors_names], 
                  df_train[,response_name], 
                  method='lasso',  metric = "RMSE", 
                  trControl=objControl)

#Get summary of features in model
objModel$trainingData
# get predictions on your testing data
predictions <- predict(object=objModel, df_test[,predictors_names])

auc <- roc(df_test[,"TARGET"], predictions)
print(auc$auc)
